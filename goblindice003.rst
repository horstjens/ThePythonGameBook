...
This *normal distribution* or *Gaussian distribution* of values is often observed in nature, and there is no reason `not` to model a medieval fantasy combat simulation using the ``random.gauss()`` function to produce `realistic` looking values. 
...
The -1 rule is added so that it becomes possible to roll a natural 6: 6-1 + 1 = 6. Witout this rule and simple rerolling, only 5 and 7 would be possible, but never 6. 
...
This is a function that can call itself, a **recursive** function or :term:`recursion`. 
