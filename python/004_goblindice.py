...
class Goblin(object):
    ...
    Each goblin has an unique number.
    Each goblin is stored in a class variable dictionary
    of the class Goblin, where the unique number is 
    the key and the class instance (the goblin itself)
    is the value."""
    number = 0 # actual unique number
    book = {}  # a dictionary holding all goblins. the number is the key
               # the goblin class instance is the value
    
    def __init__(self, name = "Noname"):
        """standard out-of-the-cave goblin, alter values after creation"""
        self.name = name
        self.number = Goblin.number # get unique number from class variable
        Goblin.number += 1
        Goblin.book[self.number] = self # store goblin into the class dictionary
        self.hitpoints = 50
        self.wins = 0
        self.min_damage = 1
        self.max_damage = 6
...
print "goblin numbers:"
print stinky.number, grunty.number, Goblin.number, Goblin.book.keys()
