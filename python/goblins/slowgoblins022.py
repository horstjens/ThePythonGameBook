...
class Goblin(object):
    ...
    def calculate_value(self):
        ...
        diffs = (pts - cfg for pts, cfg in zip((self.attack, self.defense, self.hitpoints), (Game.attack, Game.defense, Game.hitpoints)))
...
def buy_goblin(team_number):
    ...
    if Config.gold[team_number] <= 0:
        ...
        return
    new_name = generic_input(
        typ="text",
        prompt="nickname for new goblin?",
...
def show_goblins(team_number):
    ...
    for goblin in sorted(sortlist, key=operator.attrgetter(Config.sortorder[0], Config.sortorder[1], Config.sortorder[2]), reverse=Config.reverse):
...
def toggle_sleep(team_number):
    ...
    print(p)
    sleepnumber = generic_input(
        "int", "(-1 is cancel) >", -1, -1, Goblin.number
    )
    if sleepnumber == -1:
    ...
    if scoreA > scoreD:
        ...
        if defender.hitpoints <= 0:
            ...
            striketext.append(
                ...
                " ( {} had {} ".format(defender.name, defender.victory) +
                " before he got down)"
            )
...
def fight(a=0, b=1):
    ...
    for t in [a, b]:
        ...
        for goblin in tl:
            ...
            text.append(
                "{:>20}{:7.2f}{:7.2f}{:7.2f}{:7.2f}{:7.2f}".format(
...
class Team(object):
    ...
    def addmenu(self):
        ...
        return {
            self.menukey: [
                ...
                ["toggle sleep for goblin (number)", lambda: toggle_sleep(self.number)],
    ...
    def __repr__(self):
        return "team: {} has {} gold, has won {} and lost {} battles.".format(self.name, self.gold, self.won, self.lost)
...
class Game(object):
    ...
    def compare_teams(self):
        ...
        for stat in [
            "attack", "defense", "hitpoints", "value", "sleep", "victory",
            "lost", "fights", "damage_received", "damage_dealt"
        ]:
            statsum = {a: 0, b: 0}
            for x in [a, b]:
