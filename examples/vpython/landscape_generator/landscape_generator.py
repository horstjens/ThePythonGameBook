# inspiration: https://github.com/ragnraok/RandomFractalTerrain-Vpython
# install via pip: https://github.com/buckinha/DiamondSquare
import vpython as v
import random
import math
...
class Game:
    ...
    fractal = None # final object
    scene1 = v.canvas(title=f"fractal world {world_size} x {world_size}, roughness {roughness}",
                      width=1800,
                      height=700,
    ...
    # globals
    #label_sea_level = None
    #slider_sea_level = None
    #label_snow_level = None
    #label_sea_level = None
...
def func_roughness(s):
    ...
    #try:
    Game.fractal.visible = False
    Game.fractal.delete()
    #except:
    #    print("could not delete fractal")
    recalc_y_values()
    Game.busy = False
...
def repaint_fractal():
    ...
    # do NOT change height values of fractal, only change color values
    ...
    try:
        ...
        print("could not delete non-existing fractal...")
    create_fractal_from_points()
...
def create_fractal_from_points():
    # --- create quads
    quadlist = []
    ...
    Game.fractal = v.compound(quadlist,  origin=v.vector(0,0,0), pos=v.vector(0,0,0))
...
def get_color_from_y(y_value):
    ...
    if y_value < Game.sea_level:
        ...
        c = 1 - (y_value - Game.snow_level) / ( Game.max_y - Game.snow_level)
...
def make_water():
    ...
    Game.water = v.box(color=v.vector(0, 0, 1),
               opacity=0.45,
               #pos=v.vector(0, Game.min_y + Game.sea_level * (Game.max_y - Game.min_y), 0),
               #size=v.vector(Game.world_size * 2, 0.01, Game.world_size * 2),
               #pos=v.vector(Game.world_size/2, starty + (Game.sea_level * (endy - starty)/2), Game.world_size/2),
